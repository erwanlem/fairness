...
\begin{document}
    ...
    \textit{Dice} nous a permis d'établir quels attributs influent sur la prédiction de notre modèle. Les résultats sont 
    ...
    On reconnaît tout de suite les 
    attributs dont l'\textit{accuracy} est au plus haut en regardant ceux qui s'effondrent le plus. En premier lieu, l'âge s'illustre
    ...
    On remarque ici que les attributs sont plutôt centrés autour de \textit{0.5} et que la plupart d'entre eux restent rectilignes, même si certains divergent
    ...
    plus important et une origine située vers \textit{0.68}. Ce qui laisse à penser que cette version était belle et bien trop modelée sur le train
    set, l'équilibre était rompu.

    Pour conclure, BlackBoxAuditing nous a permis de comprendre quels étaient les attributs les plus discriminés. \textit{âge} est certainement le plus notable.
    ...
    attributs ne devraient pas contribuer autant à la sortie de notre modèle. Ils peuvent constituer un biais (ce n'est 
    pas parce qu'on a un accident à l'âge de 21 ans qu'on va forcément mourir, d'autres attributs sont beaucoup plus 
    importants).
    ...
    contrairement à ce que nous pouvions penser au départ.

    Ci-dessous un résultat obtenu du calcul des valeurs de Shapley. Vous pouvez retrouver un échantillon des résultats dans l'annexe \ref{appendix:shapley} de ce rapport.
    ...
    d'autres attributs sensibles qui eux ont un réel impact sur les sorties du modèle. Les attributs \textit{âge}, \textit{dep} et 
    ...
    \subsection{Une nouvelle visualisation des données}
    Dans la perspective d'avoir une vue globale des corrélations des attributs de notre dataset, nous avons décidé d'en réaliser la 
    matrice de corrélation. La figure~\ref{fig:corr_matrix} (ici peu lisible, voir dans \verb|analyze.ipynb| pour les chiffres réels) 
    nous montre que les corrélations sont globalement assez faibles. On note quelques attributs qui ressortent comme \textit{agg} 
    et \textit{vma} ou encore \textit{vma} et \textit{catr}. Il y a en effet un lien de causalité évident entre ces attributs.
    L'attribut \textit{mortal} est quant à lui corrélé avec \textit{agg} et \textit{vma}. L'attribut 
    \textit{sexe\_conducteur} est globalement très peu corrélé et devrait par conséquent avoir une faible 
    incidence sur les prédictions.
    ...
    sensible. Elle est testée sur différents seuils de prédiction afin de trouver le modèle le plus efficace.
    On note des résultats parfois moins bons avec le post-processing (meilleure courbe avant le post-processing).

    \subsubsection{Les résultats obtenus}
    La figure~\ref{fig:EQU_odds} nous montre l'évolution de la balanced accuracy et de l'equal opportunity difference de l'attribut sensible 
    ...
    Le résultat obtenu nous montre que le meilleur seuil est d'environ $0.15$. Il permet de garder un juste milieu entre la 
    balanced accuracy et l'equal oppotunity difference. La figure~\ref{fig:EQU_odds1} nous montre également que ce seuil 
    permet d'avoir un disparate impact supérieur à $0.8$ et une accuracy d'environ $0.70$. Les résultats obtenus avec cette 
    méthode ne sont pas les meilleurs mais on arrive tout de même à trouver un juste milieu afin d'avoir des valeurs acceptables dans 
    toutes les métriques.
    
    Enfin, nous avons pu remarquer que cette méthode semble plus sujette à des variations dû à l'aléatoire. 
    D'une exécution à l'autre on peut en effet avoir des résultats assez différents.
    

    \subsubsection{Une vue globale du modèle}
    Cette méthode n'apporte pas de meilleurs résultats. Elle nous montre cependant l'évolution des résultats avec 
    une accuracy croissante lorsque le seuil s'approche de $1.0$. On remarque également que la balanced accuracy 
    a un niveau acceptable lorsque le seuil est inférieur à $0.5$ mais elle est gandement réduite avec l'augmentation du seuil.
    On comprend ainsi bien le comportement du modèle: lorsqu'on augmente le seuil, 
    on réduit les prédictions de la classe vrai (mortel) mais on augmente les prédictions non mortels. Ainsi 
    étant donné qu'il y a beaucoup plus de prédictions non mortel l'accuracy augmente. Lorsqu'on réduit le seuil
     l'accuracy est réduite mais on prédit mieux les accidents mortels donc une meilleure balanced accuracy.
    
    \vspace{1cm}
    \begin{figure}[h]
        \centering
        \begin{subfigure}{7cm}
            \includegraphics[width=7cm]{./img/cal_equ_odds.png}
            \caption{Balanced accuracy (bleu) -- Equal opportunity difference (rouge)}\label{fig:EQU_odds}
        \end{subfigure}
        \hspace{0.2cm}
        \begin{subfigure}{7cm}
            \includegraphics[width=7cm]{./img/cal_equ_odds1.png}
        \caption{Accuracy (bleu) -- Disparate impact (rouge)}\label{fig:EQU_odds1}
        \end{subfigure}
